<template>
  <div
    ...
    @dblclick="fillParent"
  >
    <div class="rotate-icon" :style="{ display: enabled ? 'block' : 'none'}">旋转</div>
    <div
...
export default {
  ...
  mounted: function () {
    ...
    console.log('vue-drage-resizable ---  mounted!')
    console.log(this.$el)
  ...
  methods: {
    ...
    handleMove: function (e) {
     ...
     //move一次 相差一贞
      ...
      if(this.rotating){
        var circle={
          x:this.elmW/2 + this.elmX,
          y:this.elmH/2 + this.elmY
        }
        const currentAngle = Math.atan2(this.mouseY - circle.y, this.mouseX - circle.x)
        const lastAngle = Math.atan2(lastMouseY - circle.y, lastMouseX - circle.x)
        this.elmR   += Math.round((currentAngle - lastAngle) * 180 / Math.PI)
        this.rotate +=  Math.round((currentAngle - lastAngle) * 180 / Math.PI)
    ...
    handleUp: function (e) {
      ...
      this.elmX = this.left
      this.elmY = this.top
      this.elmR = this.rotate
    }
  },

  computed: {
    ratio:function(){
      console.log('computed当前的比例是：',this.w/this.h)
